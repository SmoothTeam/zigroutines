...
fn runTcpEcho(io_kind: zr.Config.IoConfig) !void {
    if (comptime !zr.context.supported) return error.SkipZigTest;
    if (!supportsPollNet()) return error.SkipZigTest;
    ...
    const S = struct {
        ...
        fn client(io: zr.IoBackend, pch: *PortCh, rch: *ResultCh) void {
            const p = pch.recv() catch {
                rch.send(false) catch {};
                return;
            };
        ...
        fn driver(r: *zr.Runtime, io: zr.IoBackend, pch: *PortCh, rch: *ResultCh) void {
            ...
            _ = r.spawn(.{}, client, .{ io, pch, rch }) catch {
                rch.send(false) catch {};
                return;
            };
            const outcome = zr.select.recv(bool, rch, .{
...
test "tcp echo loopback via iocp" {
    if (builtin.os.tag != .windows) return error.SkipZigTest;
    try runTcpEcho(.iocp);
}

test "udp loopback via iocp" {
    if (comptime !zr.context.supported) return error.SkipZigTest;
    if (builtin.os.tag != .windows) return error.SkipZigTest;
...
test "iocp cancelAll unblocks accept" {
    if (comptime !zr.context.supported) return error.SkipZigTest;
    if (builtin.os.tag != .windows) return error.SkipZigTest;
...
test "tcp echo loopback via io_uring" {
    if (builtin.os.tag != .linux) return error.SkipZigTest;
    try runTcpEcho(.io_uring);
}

test "udp loopback via io_uring" {
    if (comptime !zr.context.supported) return error.SkipZigTest;
    if (builtin.os.tag != .linux) return error.SkipZigTest;
...
test "io_uring cancelAll unblocks accept" {
    if (comptime !zr.context.supported) return error.SkipZigTest;
    if (builtin.os.tag != .linux) return error.SkipZigTest;
