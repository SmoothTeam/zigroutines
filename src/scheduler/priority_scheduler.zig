...
pub const PriorityScheduler = struct {
    allocator: std.mem.Allocator,
    ready: pq_mod.PriorityQueues(*Task),
    ...
    pub fn init(allocator: std.mem.Allocator) PriorityScheduler {
        return .{
            .allocator = allocator,
            .ready = pq_mod.PriorityQueues(*Task).init(allocator),
        };
    }

    pub fn deinit(self: *PriorityScheduler) void {
    ...
    pub fn executor(self: *PriorityScheduler) executor_mod.Executor {
    ...
    pub fn enqueue(self: *PriorityScheduler, t: *Task) !void {
        ...
        try self.ready.push(t.priority, t);
    }

    pub fn noteSpawn(self: *PriorityScheduler) void {
        self.live += 1;
    }

    pub fn run(self: *PriorityScheduler) void {
        self.stopped = false;
        while (!self.stopped) {
            if (self.timers) |tq| {
                _ = tq.fireExpired();
    ...
    pub fn stop(self: *PriorityScheduler) void {
        self.stopped = true;
    }
    ...
    pub fn collectUnjoined(self: *PriorityScheduler) void {
